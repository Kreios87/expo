...
> **info** Prevent selection is available in SDK 56 and later.

You can prevent native selection of a tab by setting the `preventSelection` prop on the `NativeTabs.Trigger` component. When `true`, tapping the tab in the tab bar does not change the focused tab.

> **Note:** Only native tab selection is suppressed. JavaScript navigation (for example, `router.push('/settings')` or `<Link href="/settings" />`) still navigates to the tab.
...
export default function TabLayout() {
  return (
    <NativeTabs>
      ...
      <NativeTabs.Trigger name="settings" preventSelection>
        <NativeTabs.Trigger.Label>Settings</NativeTabs.Trigger.Label>
      </NativeTabs.Trigger>
    </NativeTabs>
  );
...
You can also toggle `preventSelection` dynamically from inside a screen.
...
export default function CheckoutScreen() {
  ...
  return (
    <View>
      <NativeTabs.Trigger preventSelection={isProcessing} />
      {/* ... */}
    </View>
  );
}
